...
def log_likelihood(y_true, para_pred):
    likelihood = gaussian(y_true[:,:,0], para_pred[:, :, 0], para_pred[:, :, 1])
...
def train_model(window_size, LSTM_neurons):
    main_input = Input(shape=(window_size, 7), name = 'main_input')
    series_embed = Dense(embed_size, activation = 'relu', kernel_initializer = 'truncated_normal', input_shape = (num_series,))(main_input)
...
for i in range(tot_epoch):
    ...
    for j in range(nb_epoch):
        ...
        data_values[chosen_batch, :, 2:-1], scale_values[chosen_batch, 0]], data_values[chosen_batch, :, 1].reshape(64, window_size, 1))))
    ...
    #plt.pause(5)
    f.savefig("elect_no_loss_"+str(i)+".png")
    plt.close()
...
f.savefig("elect_no_loss_RMSE_ND.png")
plt.close()
