...
class DeepAR (object):
	def __init__(self,
		...
		with self.graph.as_default():
			...
			self.cnn_output = tf.concat([cnn_output_padding,cnn_output], axis=1) #[batch_size, window_size, hidden_unit]
			...
			cnn_output_average = tf.reduce_mean(self.cnn_output, axis=1) #
			#take average of CNN_output in encode_range as CNN_putput in precdict range since CNN_putput in precdict range should not be known
			self.cnn_output_pad = tf.concat([tf.expand_dims(cnn_output_average, axis=1) for i in range(decode_length)], axis=1) # [batch_size, decode_length, 40]
			self.cnn_output_pred = tf.concat([self.cnn_output[:, :encode_length, :],self.cnn_output_pad], axis=1) #[batch_size, window_size, 40]
			...
			def RMSE(mu, label, std):
				...
				std_pred = std[:, self.encode_length:self.window_size]
			...
			self.RMSE_train = RMSE(self.miu_concat, self.input_y, self.sigma_concat)
			self.ND_train = ND(self.miu_concat, self.input_y, self.sigma_concat)


	##########################.   Prediction.     ############################
			# update input for prediction since CNN_output is different in prediction
			self.lstm_input_with_cnn = tf.concat([self.cnn_output_pred, self.lstm_input], axis = 2) #[batch_size, 192, 40+3]
			...
			def miu_pred(output_pred, v_pred, W_pred, b_pred): #output_pred.shape: [1, hidden_unit]
				...
				# here miu has not be scaled back
				return miu_temp
			...
			def decode_input(self, decode_input_value,time_step, keep_prob):
				...
				cnn_output_latest_squeeze = tf.squeeze(cnn_output_latest, axis=1) #[batch_size, hidden_unit]
				decode_input_data = tf.concat([cnn_output_latest_squeeze,
												decode_input_value,
												self.lstm_input_with_cnn[:,time_step,self.hidden_unit+1:]], axis=1)
				return decode_input_data, cnn_output_latest

			cnn_output_pred_list = []
			...
			with tf.variable_scope("RNN", reuse = True):
				...
				decode_input_data, cnn_output_pred_latest = decode_input(self, decode_input_value, self.encode_length, self.keep_prob) #更新decode第一个input的数值
				# add cnn_output in prediction range to list
				cnn_output_pred_list.append(cnn_output_pred_latest)
				for time_step_decode in range(self.encode_length, self.window_size):
					...
					if time_step_decode < (self.window_size-1):
						decode_input_data, cnn_output_pred_latest = decode_input(self, decode_input_value, time_step_decode+1, self.keep_prob) #更新time_step_decode+1 的 input的数值
						cnn_output_pred_list.append(cnn_output_pred_latest)
			...
			#concate cnn_output in prediciton range and concate with cnn_output in encode_range
			cnn_output_pred = tf.concat(cnn_output_pred_list, axis=1) # [batch_size, decode_length, hidden_unit] different from self.cnn_output_pred in previous
			print ("cnn_output_pred.shape: ", cnn_output_pred.shape) #[batch_size, decode_length, hidden_unit]
			self.cnn_output_all = tf.concat([self.cnn_output[:, :self.encode_length, :], cnn_output_pred], axis=1)
			print ("self.cnn_output_all.shape: ", self.cnn_output_all.shape) #[batch_size, window_size, hidden_unit]
			...
			self.RMSE_pred = RMSE(self.miu_concat_pred, self.input_y, self.sigma_concat_pred)
			self.ND_pred = ND(self.miu_concat_pred, self.input_y, self.sigma_concat_pred)
			...
			tf.add_to_collection("CNN_output", self.cnn_output_all)
	...
	def train(self, checkpoint_dir):
		...
		def plot(label, prediction,train,num_plot, step):
			...
			for i in range(num_plot):
				...
				plt.axvline(self.encode_length, color='k', linestyle = "dashed")
				#参考线
		...
		def train_step(x_batch, y_batch,v_batch, batch_size,forget_gate_mask,keep_prob=0.7):
		...
		def pred_step(x_batch, y_batch, v_batch, batch_size,forget_gate_mask, keep_prob=1.0):
			#x_batch = np.concatenate((forget_gate_mask, x_batch), axis = 1) #[batch_size, 40+24]
		...
		with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options), graph=self.graph) as sess:
			init = tf.global_variables_initializer()
			sess.run(init)
			#创建saver对象
			saver = tf.train.Saver()
			forget_gate_mask_sample = np.full((64, 168*2, self.hidden_unit), 1, dtype = np.float32)
			...
			for j in range(20): #epochs
				for index in self.index_list:
					...
					if step%2000==0:
						...
						#print (miu_pred)
						saver.save(sess, '../../checkpoint/checkpoint_CNN_Fuahui_v_40_long_prediction_new/' + checkpoint_dir + '/DeepAR_model',global_step = step)
...
for i in range(num_series):
	shift_train_data_series = shift_train_data[i]
	v_all_series = v_all[i]
	shift_train_label_series = shift_train_label[i]
	index_list_series = index_list[i]
	indexs_pred_list_series = indexs_pred_list[i]

	Model = DeepAR(
				 input_x_all=shift_train_data_series, #shape:[num_window*num_series(1453*370), window_size,4]
				 input_y_all=shift_train_label_series, #shape: [num_window*num_series(1453*370),window_size]
				 v_all = v_all_series, #shape: [num_window*num_series(1453*370) , 1]
				 index_list = index_list_series,
				 indexs_pred = indexs_pred_list_series,
	...
	Model.train(checkpoint_dir = "series"+str(i))
