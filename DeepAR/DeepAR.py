	...
	def __init__(self,
		...
		with tf.variable_scope("RNN"):
			for time_step in range(self.window_size):
				...
				(cell_output, state) = cell(self.lstm_input[:,time_step,:], state) # 按照顺序向cell输入文本数据
...
##########################.   Prediction.     ############################
		...
		def decode_input(decode_input_value,time_step):
			...
			decode_input_data = tf.concat([decode_input_value, self.lstm_input[:,time_step,1:]], axis=1)
		...
		with tf.variable_scope("RNN", reuse = True):
			for time_step_encode in range(self.encode_length): 
				#cell_output:[batch_size, hidden_unit], state=(c,h)
				(cell_output, state_pred) = cell(self.lstm_input[:,time_step_encode,:], state_pred) # 按照顺序向cell输入文本数据
				#print ('self.lstm_input[:,time_step_encode,:].shape: ', self.lstm_input[:,time_step_encode,:].shape)
				cell_output = tf.nn.relu(cell_output)
				...
				#print ('miu1.shape: ', miu_pred(cell_output, self.v, W_pred, b_pred).shape)
			...
			for time_step_decode in range(self.encode_length, self.window_size):
				(cell_output, state_pred) = cell(decode_input_data, state_pred) # 按照顺序向cell输入文本数据
				cell_output = tf.nn.relu(cell_output)
				outputs_pred.append(cell_output)
				decode_input_value = miu_pred(cell_output, self.v, W_pred, b_pred)
				#print ('miu2.shape:', decode_input_value.shape)
	...
	def train(self):
		...
		def train_step(x_batch,onehot_batch, y_batch,v_batch, batch_size):
			feed_dict = {
		...
		def pred_step(x_batch, onehot_batch, y_batch, v_batch, batch_size):
			feed_dict ={
			self.input_x: x_batch, 
			self.input_x_onehot: onehot_batch,
			self.input_y: y_batch,
