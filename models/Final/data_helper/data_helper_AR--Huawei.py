...
def read(data_file_list):
	...
	for i, day in enumerate(day_list):
	    if i==0:
	        day_list[i] = day_of_the_week[0:num_days_list[0]]
	        hour_list[i] = hour_of_the_day[0:num_days_list[0]]
	    else:
...
def prepare():
	...
	ar_data = []#train data as fitted by autoregression
	...
	for data_cov_tuple in data_cov_tuple_list:
		...
		for serie in range(num_series):	 # loop through all time series
			for i in range(num_window):
				...
				if i == 0:
					if np.isin([serie*num_window+i],indexs_pred,assume_unique=False):
						ar = ar_model.AR(np.array(data[(:encode_length,serie]))
					else:
						ar = ar_model.AR(np.array(data[(:window_size,serie]))
					model_ar = ar.fit()
					temp_ar_data = np.zeros(window_size)
					temp_ar_data[24:], detect_error = model_ar.predict(start = 24, end = window_size-1)
					temp_ar_data[:24] = temp_ar_data[24]
				else if np.isin([serie*num_window+i],indexs_pred,assume_unique=False):
					ar = ar_model.AR(np.array(data[(i-1)*24:i*24+encode_length,serie]))
					model_ar = ar.fit()
					temp_ar_data, detect_error = model_ar.predict(start = 24, end = 23+window_size)
				else:
					ar = ar_model.AR(np.array(data[(i-1)*24:i*24+window_size,serie]))
					model_ar = ar.fit()
					temp_ar_data, detect_error = model_ar.predict(start = 24, end = 23+window_size)
				ar_data.append(temp_ar_data.reshape([1, window_size]))
...
np.save("../data/huawei_z/shift_train_data.npy",shift_train_data)
np.save("../data/huawei_z/shift_train_onehot.npy",shift_train_onehot)
np.save("../data/huawei_z/v.npy",v)
np.save("../data/huawei_z/mean.npy", means)
np.save("../data/huawei_z/shift_train_label.npy",shift_train_label)
np.save("../data/huawei_z/param.npy", param)
np.save("../data/huawei_z/indexs_list.npy", indexs_list)
np.save("../data/huawei_z/indexs_pred_list.npy", indexs_pred_list)
'''
