...
class DeepAR (object):
	def __init__(self,
		...
		self.input_ar_all = input_ar_all
		...
		self.input_ar = tf.placeholder(
			tf.float32, [None,window_size], name = 'input_ar') # [None, 192]
		...
		self.cnn_output_pad = tf.concat([tf.expand_dims(cnn_output_average, axis=1) for i in range(decode_length)], axis=1) # [batch_size, decode_length, 10]
		...
		with tf.variable_scope("Miu"):
			...
			for output in outputs:
				miu_temp = (tf.nn.xw_plus_b(output, W, b, name='miu')) #
				miu.append(miu_temp)

			#concate miu in all timesteps
			self.miu_concat = tf.concat(miu, axis=1) # [None, window_size]
			print ("self.miu_concat.shape: ", self.miu_concat.shape )
			self.miu_concat = self.miu_concat +  self.input_ar
			self.miu_concat = tf.multiply(self.miu_concat, self.v) #还原scale
...
##########################.   Prediction.     ############################
		...
		with tf.variable_scope("RNN", reuse = True):
			for time_step_encode in range(self.encode_length):
				...
				miu_pred_list.append(miu_pred(cell_output, self.v, W_pred, b_pred)) ##update list of miu
			...
			for time_step_decode in range(self.encode_length, self.window_size):
				...
				miu_pred_list.append(decode_input_value)#update list of miu
			...
			# Add AR
			self.miu_concat_pred = self.miu_concat_pred + self.input_ar
			# scale back
			self.miu_concat_pred = tf.multiply(self.miu_concat_pred, self.v)
	...
	def train(self):
		...
		def train_step(x_batch,onehot_batch, y_batch, ar_batch, v_batch, batch_size,forget_gate_mask,keep_prob=0.7):
		...
		def pred_step(x_batch, onehot_batch, y_batch, ar_batch, v_batch, batch_size,forget_gate_mask, keep_prob=1.0):
			#x_batch = np.concatenate((forget_gate_mask, x_batch), axis = 1) #[batch_size, 40+24]
		...
		with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
			...
			for j in range(20): #epochs
				for index in self.index_list:
					...
					input_ar_batch = self.input_ar_all[index]
					...
					if step%2000==0:
						...
						for index_pred in self.indexs_pred:
							...
							input_ar_batch_pred = self.input_ar_all[index_pred]
						...
						#print (miu_pred)
						saver.save(sess, '../../checkpoint/checkpoint_CNN_AR_v_40/DeepAR_model',global_step = step)
...
shift_train_data_ar = np.load("../../data/huawei/shift_train_data_AR.npy")
...
Model = DeepAR(
				input_x_all=shift_train_data, #shape:[num_window*num_series(1453*370), window_size,4]
				 ...
				 input_ar_all=shift_train_data_ar,
