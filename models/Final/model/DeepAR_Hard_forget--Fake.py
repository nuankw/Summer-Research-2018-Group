...
##### Current change: single layer lSTM and plot forget_gate
...
##########################.   Prediction.     ############################
		...
		with tf.variable_scope("RNN", reuse = True):
			...
			#compute norm of forget_gate_all
			forget_gate_all_norm = tf.norm(self.forget_gate_all, ord='euclidean', axis=2) #[batch_size, 192]
			#scale forget_gate_all
			forget_gate_all_mean = tf.reduce_mean(forget_gate_all_norm, axis=1) # [batch_size, ]
			forget_gate_all_mean_stack = tf.concat([tf.expand_dims(forget_gate_all_mean, axis=1) for i in range(self.window_size)] , axis=1)
			self.forget_gate_scale = tf.divide(forget_gate_all_norm, forget_gate_all_mean_stack) #[64, 192]
			print ("forget_gate_scale.shape: ", self.forget_gate_scale.shape)
			#miu_concat_pred=tf.reshape(miu_concat_pred,[None, self.window_size,1])
	...
	def train(self):
		...
		def pred_step(x_batch, onehot_batch, y_batch, v_batch, batch_size,forget_gate_mask, keep_prob=1.0):
			...
			neg_log_likelihood_pred, prob_log_pred,RMSE_pred, ND_pred, miu_pred, forget_gate_scale_pred = sess.run([
			 			 										self.neg_log_likelihood_pred,
			 			 										self.prob_log_pred,
			 			 										self.RMSE_pred,
			 			 										self.ND_pred,
			 			 										self.miu_concat_pred,
																self.forget_gate_scale,
			 			 										],
			 			 										feed_dict = feed_dict)
			return (neg_log_likelihood_pred, RMSE_pred, ND_pred, miu_pred, forget_gate_scale_pred)
				...
				for index in self.index_list:
					...
					if step%500==0:
						...
						for index_pred in self.indexs_pred:
							...
							neg_log_likelihood_pred, RMSE_pred, ND_pred, miu_pred, forget_gate_scale_pred = pred_step(input_x_batch_pred,
						...
						#print (miu_pred)
						saver.save(sess, '../../checkpoint/checkpoint_forget_Fuahui_single_layer/DeepAR_model',global_step = step)
					...
					if step%2000 == 0:
						...
						plot(input_y_batch_pred, miu_pred, forget_gate_scale_pred, 8, step)
						print ("Update 1.png")
